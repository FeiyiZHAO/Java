...
public class NiveauFacile {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					NiveauFacile window = new NiveauFacile();
			 ...
			 public void actionPerformed(ActionEvent e)
			 {
				...
				new	Thread(new	Runnable()	{
	    			...
	    			public void	run()	{
	    				...
	    				{
	    					if(etu[j].getEtat()==1) //Si le professeur peut effectuer une action pour un étudiant mise en couleur du bouton
	    					{
	...
	public void actionDeplacementEtuCons(int etu)
	{
		for(int i=0;i<niv.getNbrC();i++)
		{
			if(cons[i].getLibreEtu()) //Si la console est libre 
			{	
